...
# $Id: check_netappfiler.py 69 2008-12-02 16:24:51Z svelt $
...
# 1st: Build command line options from parameters
if options.version == '1' or options.version == '2c':
	# cmdline = '/usr/bin/snmpget -v1 -c%s -OqevtU %s %s 2>/dev/null' % (options.community, options.host, oid)
	CMDLINE = '/usr/bin/snmpget -v%s -c%s -OqevtU %s %%s 2>/dev/null' % (options.version, options.community, options.host)
elif options.version == '3':
	# cmdline = '/usr/bin/snmpget -v1 -c%s -OqevtU %s %s 2>/dev/null' % (options.community, options.host, oid)
	CMDLINE = '/usr/bin/snmpget -v3 -OqevtU -l%s -u%s -a%s -A%s %s %%s 2>/dev/null' % (options.seclevel, options.secname, options.authproto, options.authkey, options.host)
else:
	print "Unknown protocol version (-P) %s " % options.version
	sys.exit(127)

if options.verb >=3:
	print "Using commandline " + CMDLINE
